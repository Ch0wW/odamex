...
bool SV_AddMaster(const char *masterip)
{
	...
	for(size_t i = 0; i < masters.size(); i++)
	{
		...
		{
			Printf(PRINT_WARNING, "Master %s [%s] is already on the list", m.masterip.c_str(), m.masteraddr.ToString());
			return false;
		}
	}
	...
	{
		Printf(PRINT_WARNING, "Failed to resolve master server: %s, not added", m.masterip.c_str());
		return false;
	}
	else
	{
		Printf(PRINT_WARNING, "Added master: %s [%s]", m.masterip.c_str(), m.masteraddr.ToString());
		masters.push_back(m);
	}
...
bool SV_RemoveMaster(const char *masterip)
{
	for(size_t index = 0; index < masters.size(); index++)
	{
		...
		{
			Printf(PRINT_WARNING, "Removed master server: %s", masters[index].masterip.c_str());
			masters.erase(masters.begin() + index);
			return true;
		}
	}

	Printf(PRINT_WARNING, "Failed to remove master: %s, not in list", masterip);
	return false;
}
