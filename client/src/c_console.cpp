...
static int C_PrintString(int printlevel, const char* color_code, const char* outline)
{
	...
	// Ch0wW : THIS PART BELOW IS UGLY UGLY UGLY
	if (color_code && color_code[2] != '+' && printlevel >= 0 && (printlevel < PRINTLEVELS || printlevel == PRINT_WARNING || printlevel == PRINT_ERROR))
	{
...
int STACK_ARGS Printf(const char *format, ...)
{
	...
	va_start(argptr, format);
	int count = VPrintf(PRINT_HIGH, TEXTCOLOR_NORMAL, format, argptr);
	va_end(argptr);
