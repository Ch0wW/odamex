...
void ZDoomHUD() {
	...
	{	// Draw health
		...
		{
			curr_powerup = medi[1];
			yPos = 6;	// the y position of the Berzerk is slightly lowered by 4. So make it the same y position as the medikit.
		}

		if (hud_scale)
			screen->DrawLucentPatchCleanNoMove(curr_powerup, 20 * CleanXfac,
				I_GetSurfaceHeight() - yPos * CleanYfac);
		else
			screen->DrawLucentPatch(curr_powerup, 20, I_GetSurfaceHeight() - yPos);
