...
static std::list<MidiEvent*> *I_ReadMidiTrack(MEMFILE *mf)
{
	...
	{
		Printf(PRINT_ERROR, "I_ReadMidiTrack: Unexpected chunk header ID\n");
		return NULL;
	}
	...
	while (mem_ftell(mf) < int(trackend))
	{
		...
		if (!newevent)
		{
			Printf(PRINT_ERROR, "I_ReadMidiTrack: Unable to read MIDI event\n");
...
void MidiSong::_ParseSong(MEMFILE *mf)
{
	...
	{
		Printf(PRINT_ERROR, "MidiSong::_ParseSong: Unexpected file header ID\n");
		return;
	}
	...
	{
		Printf(PRINT_ERROR, "MidiSong::_ParseSong: Only type 0 or type 1 MIDI files are supported.\n");
		return;
	}
	...
	for (size_t i = 0; i < fileheader.num_tracks; i++)
	{
		...
		if (!eventlist)
		{
			Printf(PRINT_ERROR, "MidiSong::_ParseSong: Error reading track %d.\n", i + 1);
			return;
		}
